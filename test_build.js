...
(function () {
    ...
    checkPromise.then(() => {
      ...
      changelogBlock.innerHTML = `
        ...
        <div class="changelog-content">
          <ul>
            <li><strong>v1.50</strong> - Исправлены ошибки загрузки Turbo и Lumex плееров</li>
  ...
  // ИСПРАВЛЕННАЯ ФУНКЦИЯ ДЛЯ TURBO
  async function loadTurboPlayer(id, episode) {
    ...
    // Пытаемся получить данные из Kodik API
    const kodikCacheKey = `kodik_${id}`;
    let kodikData = getCachedData(kodikCacheKey);
    if (!kodikData) {
      try {
        ...
        // Не прерываем выполнение, а пробуем альтернативный метод
      }
    }
    
    // Альтернативный метод: прямой запрос к Turbo
    try {
      const turboUrl = `https://turbo.to/embed/shikimori/${id}?episode=${episode}`;
      debugLog(`Пробуем прямой URL для Turbo: ${turboUrl}`);
      
      // Проверяем доступность URL
      const response = await gmGetWithTimeout(turboUrl, {
        headers: {
          Referer: "https://turbo.to/",
          Origin: "https://turbo.to",
        }
      });
      
      if (response && response.length > 0) {
        setCachedData(cacheKey, turboUrl);
        debugLog("Turbo плеер успешно загружен через прямой URL");
        return turboUrl;
      }
    } catch (error) {
      debugLog("Ошибка при загрузке Turbo через прямой URL:", error);
    }
    
    // Если прямой метод не сработал, пробуем через Kinobox
    if (kodikData && kodikData.results && kodikData.results.length > 0) {
      const { kinopoisk_id, worldart_id, shikimori_id } = kodikData.results[0];
        ...
        try {
          ...
          if (kinoboxData.data && Array.isArray(kinoboxData.data)) {
            ...
            if (turboPlayer?.iframeUrl) {
              setCachedData(cacheKey, turboPlayer.iframeUrl);
              debugLog("Turbo плеер успешно загружен через Kinobox API");
              return turboPlayer.iframeUrl;
            }
          }
        } catch (error) {
          debugLog("Ошибка при загрузке Turbo через Kinobox API:", error);
        }
      }
    }
    
    // Если все методы не сработали, пробуем последний вариант
    try {
      const fallbackUrl = `https://turbo.to/player/shikimori/${id}/${episode}`;
      debugLog(`Пробуем запасной URL для Turbo: ${fallbackUrl}`);
      setCachedData(cacheKey, fallbackUrl);
      return fallbackUrl;
    } catch (error) {
      debugLog("Ошибка при использовании запасного URL для Turbo:", error);
    }
    
    // Если ничего не помогло, выбрасываем ошибку
    debugLog("Не удалось загрузить Turbo плеер");
    showNotification("Не удалось загрузить Turbo плеер. Попробуйте другой плеер.", "error");
    throw new Error("Не удалось загрузить Turbo плеер");
  }
  
  // ИСПРАВЛЕННАЯ ФУНКЦИЯ ДЛЯ LUMEX
  async function loadLumexPlayer(id, episode) {
    ...
    // Альтернативный метод: прямой запрос к Lumex
    try {
      const lumexUrl = `https://lumex.pro/embed/shikimori/${id}?episode=${episode}`;
      debugLog(`Пробуем прямой URL для Lumex: ${lumexUrl}`);
      
      // Проверяем доступность URL
      const response = await gmGetWithTimeout(lumexUrl, {
        headers: {
          Referer: "https://lumex.pro/",
          Origin: "https://lumex.pro",
        }
      });
      
      if (response && response.length > 0) {
        setCachedData(cacheKey, lumexUrl);
        debugLog("Lumex плеер успешно загружен через прямой URL");
        return lumexUrl;
      }
    } catch (error) {
      debugLog("Ошибка при загрузке Lumex через прямой URL:", error);
    }
        ...
        try {
          ...
          if (kinoboxData.data && Array.isArray(kinoboxData.data)) {
            ...
            if (lumexPlayer?.iframeUrl) {
              ...
              debugLog("Lumex плеер успешно загружен через Kinobox API");
              return url;
            }
          }
        } catch (error) {
          debugLog("Ошибка при загрузке Lumex через Kinobox API:", error);
        }
      }
    }
    ...
    // Если все методы не сработали, пробуем последний вариант
    try {
      const fallbackUrl = `https://lumex.pro/player/shikimori/${id}/${episode}`;
      debugLog(`Пробуем запасной URL для Lumex: ${fallbackUrl}`);
      setCachedData(cacheKey, fallbackUrl);
      return fallbackUrl;
    } catch (error) {
      debugLog("Ошибка при использовании запасного URL для Lumex:", error);
    }
    
    // Если ничего не помогло, выбрасываем ошибку
    debugLog("Не удалось загрузить Lumex плеер");
    showNotification("Не удалось загрузить Lumex плеер. Попробуйте другой плеер.", "error");
    throw new Error("Не удалось загрузить Lumex плеер");
  }
